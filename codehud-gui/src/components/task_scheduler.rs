...
pub struct TaskSchedulerComponent {
...
impl GuiComponent for TaskSchedulerComponent {
    fn name(&self) -> &str { "task_scheduler" }
    fn render(&mut self, ui: &mut Ui, _ctx: &Context) -> GuiResult<()> {
        if self.visible { ui.label("Task Scheduler Component"); }
        Ok(())
    }
