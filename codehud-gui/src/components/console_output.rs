...
pub struct ConsoleOutputComponent {
...
impl GuiComponent for ConsoleOutputComponent {
    fn name(&self) -> &str { "console_output" }
    fn render(&mut self, ui: &mut Ui, _ctx: &Context) -> GuiResult<()> {
        if self.visible { ui.label("Console Output Component"); }
        Ok(())
    }
