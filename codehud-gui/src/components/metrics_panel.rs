...
pub struct MetricsPanelComponent {
...
impl GuiComponent for MetricsPanelComponent {
    fn name(&self) -> &str { "metrics_panel" }
    fn render(&mut self, ui: &mut Ui, _ctx: &Context) -> GuiResult<()> {
        if self.visible { ui.label("Metrics Panel Component"); }
        Ok(())
    }
