...
pub struct GitIntegrationComponent {
...
impl GuiComponent for GitIntegrationComponent {
    fn name(&self) -> &str { "git_integration" }
    fn render(&mut self, ui: &mut Ui, _ctx: &Context) -> GuiResult<()> {
        if self.visible { ui.label("Git Integration Component"); }
        Ok(())
    }
