...
pub struct QualityDashboardComponent {
...
impl QualityDashboardComponent {
...
impl GuiComponent for QualityDashboardComponent {
    fn name(&self) -> &str { "quality_dashboard" }
    fn render(&mut self, ui: &mut Ui, _ctx: &Context) -> GuiResult<()> {
        if self.visible { ui.label("Quality Dashboard Component"); }
        Ok(())
    }
