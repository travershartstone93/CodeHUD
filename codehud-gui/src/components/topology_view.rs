...
pub struct TopologyViewComponent {
...
impl GuiComponent for TopologyViewComponent {
    fn name(&self) -> &str { "topology_view" }
    fn render(&mut self, ui: &mut Ui, _ctx: &Context) -> GuiResult<()> {
        if self.visible { ui.label("Topology View Component"); }
        Ok(())
    }
