...
pub struct SearchPanelComponent {
...
impl GuiComponent for SearchPanelComponent {
    fn name(&self) -> &str { "search_panel" }
    fn render(&mut self, ui: &mut Ui, _ctx: &Context) -> GuiResult<()> {
        if self.visible { ui.label("Search Panel Component"); }
        Ok(())
    }
