...
pub struct DocumentationViewerComponent {
...
impl DocumentationViewerComponent {
...
impl GuiComponent for DocumentationViewerComponent {
    fn name(&self) -> &str { "documentation_viewer" }
    fn render(&mut self, ui: &mut Ui, _ctx: &Context) -> GuiResult<()> {
        if self.visible { ui.label("Documentation Viewer Component"); }
        Ok(())
    }
