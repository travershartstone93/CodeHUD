...
pub struct RefactoringToolsComponent {
...
impl RefactoringToolsComponent {
...
impl GuiComponent for RefactoringToolsComponent {
    fn name(&self) -> &str { "refactoring_tools" }
    fn render(&mut self, ui: &mut Ui, _ctx: &Context) -> GuiResult<()> {
        if self.visible { ui.label("Refactoring Tools Component"); }
        Ok(())
    }
