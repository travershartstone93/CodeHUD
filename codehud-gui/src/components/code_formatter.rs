...
pub struct CodeFormatterComponent {
...
impl GuiComponent for CodeFormatterComponent {
    fn name(&self) -> &str { "code_formatter" }
    fn render(&mut self, ui: &mut Ui, _ctx: &Context) -> GuiResult<()> {
        if self.visible { ui.label("Code Formatter Component"); }
        Ok(())
    }
