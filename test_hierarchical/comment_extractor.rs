...
impl CommentExtractor {
    ...
    pub fn extract_from_file(&self, file_path: &Path) -> LlmResult<FileCommentExtraction> {
        ...
        let structural_insights = Some(self.generate_structural_insights(file_path, &content, &language));
    ...
    pub fn generate_structural_insights(&self, file_path: &Path, content: &str, language: &str) -> StructuralInsights {
        ...
        if let Ok(bullet_points) = self.generate_bullet_points_with_narrator(file_path, content, language) {
    ...
    fn convert_ts_node_recursive(&self, ts_node: &tree_sitter::Node, source: &[u8]) -> LlmResult<Node> {
        ...
        let text = if ts_node.child_count() == 0 {
            // Only leaf nodes should have text
            ts_node.utf8_text(source).ok().map(|s| s.to_string())
        } else {
            None
        };
