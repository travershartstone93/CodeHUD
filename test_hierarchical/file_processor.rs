...
impl FileProcessor {
    ...
    pub async fn generate_text_summary(&self, prompt: &str) -> LlmResult<String> {

        // Make direct HTTP call to Ollama API
        let system_prompt = "You are an expert code analyst. Provide concise, accurate analysis of code comments and their implications.";
    ...
    pub async fn generate_file_summary(&self, extraction: &crate::comment_extractor::FileCommentExtraction) -> LlmResult<FileSummary> {
        ...
        let analysis_response = self.generate_text_summary(&prompt).await?;
